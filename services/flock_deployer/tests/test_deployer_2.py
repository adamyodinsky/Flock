...
deployer, resource_store = setup_deployer()
DRY_RUN = False
...
def test_manifest_creator(target_name, target_namespace, target_kind, creator_function):
    target_manifest = resource_store.get(
    ...
    deployment_manifest = creator_function(
        name="test-agent",
        namespace="default",
        target_manifest=schema_factory.get_schema(target_manifest["kind"]).validate(
            target_manifest
        ),
    )
...
def test_manifest_creator_and_deployer(
    target_kind, target_name, target_namespace, dry_run=DRY_RUN
):
    deployment_manifest = test_manifest_creator(
    ...
    time.sleep(3)
    process_manifest(
        deployer.service_deployer, deployment_manifest, target_manifest, dry_run
    )
...
def main():
    ...
    test_manifest_creator("my-agent", "default", "Agent", manifest_creator_job)
    ...
    test_manifest_creator_and_deployer("Agent", "my-agent", "default")
